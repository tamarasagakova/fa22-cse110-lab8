describe('Basic user flow for Website', () => {
  ...
  it('Clicking the "Add to Cart" button should change button text', async () => {
    ...
    const shadow_root = await productItem.getProperty('shadowRoot');
    const button = await shadow_root.$('button');
    await button.click();
    let inner_text = await button.getProperty('innerText');
    let json = await inner_text.jsonValue();
  ...
  it('Checking number of items in cart on screen', async () => {
    ...
    for (let i = 1; i < productItems.length; i++) {
      const shadow_root = await productItems[i].getProperty('shadowRoot');
      const button = await shadow_root.$('button')
      await button.click();
    }
